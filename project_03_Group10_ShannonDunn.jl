...
# ╔═╡ 5f452d82-7d91-4e1a-a24d-9a0dc925d656
begin
    L = 0.4          # bar length, m
    ...
    I1 = 1e-6        # tiny inertia for block, since theta1 is constrained
    I2 = (1/12) * m2 * L^2  # slender rod about center of mass

    r = L / 2        # distance from bar center to pin
end

# ╔═╡ e1bef49c-82e5-424b-a931-9a138cc9547b
M = Diagonal([m1, m1, I1, m2, m2, I2])

# ╔═╡ 755f56d6-617a-4428-b942-c45a16e5bdb7
function force_vector(q, qdot)
    x1, y1, θ1, x2, y2, θ2 = q

    Q = zeros(6)

    # spring force on block
    Q[1] = -k * x1
...
# ╔═╡ 1575c7b2-9123-43c0-aecb-a22df2bc13ec
function constraints(q)
    x1, y1, θ1, x2, y2, θ2 = q

    return [
        y1,                              # block stays on horizontal track
        θ1,                              # block does not rotate
        x2 - x1 - r*cos(θ2),             # pin constraint in x
        y2 - y1 - r*sin(θ2)              # pin constraint in y
    ]
end

# ╔═╡ 04bf64a5-8ebd-40dd-aeeb-0b2a225abbce
...
# ╔═╡ d8abef3a-9e84-48ad-a530-c7307af32ace
...
# ╔═╡ ff096de5-d916-41c3-8cab-a1c4203a9df0
function solve_augmented(q, qdot)
    Cq = constraint_jacobian(q)
    Q = force_vector(q, qdot)
...
# ╔═╡ c544f9d6-497d-432a-9ce2-ef0d588513d2
function system_dynamics!(du, u, p, t)
    q = u[1:6]
    qdot = u[7:12]

    qddot, λ = solve_augmented(q, qdot)

    du[1:6] = qdot
    du[7:12] = qddot
end

# ╔═╡ 4a7c9d6f-0800-46d6-b66e-64ae8f1b0f75
begin
    θ2_0 = π/4
    ...
    x2_0 = x1_0 + r*cos(θ2_0)
    y2_0 = y1_0 + r*sin(θ2_0)

    q0 = [x1_0, y1_0, θ1_0, x2_0, y2_0, θ2_0]
...
# ╔═╡ 0f65fb76-fa95-4783-9a96-d4ce6fdd1241
begin
    tspan = (0.0, 5.0)
...
# ╔═╡ 2e1fda4b-ee53-4036-a22c-3352ad17684d
constraint_error = maximum(norm(constraints(sol.u[i][1:6])) for i in eachindex(sol.u))

# ╔═╡ 184bb619-eecc-477e-a4b0-1925945e755c
begin
    t = sol.t

    x1_vals = [u[1] for u in sol.u]
    y1_vals = [u[2] for u in sol.u]

    x2_vals = [u[4] for u in sol.u]
    y2_vals = [u[5] for u in sol.u]

    θ2_vals = [u[6] for u in sol.u]
end

# ╔═╡ 25b78ac4-7615-4e41-afc0-705fb2cf2673
plot(
    t,
    x1_vals,
    xlabel = "Time (s)",
    ylabel = "Block Position x₁ (m)",
    title = "Sliding Block Motion",
    legend = false
)

# ╔═╡ 8f5387e1-331b-4a03-83d8-daf513004363
plot(
    t,
    θ2_vals,
    xlabel = "Time (s)",
    ylabel = "Bar Angle θ₂ (rad)",
    title = "Rigid Bar Angular Motion",
    legend = false
)

# ╔═╡ ff89b1e3-6221-4790-87a4-190233d0da32
begin
    lambdas = []
    ...
    λ1_vals = [λ[1] for λ in lambdas]
    λ2_vals = [λ[2] for λ in lambdas]
    λ3_vals = [λ[3] for λ in lambdas]
    λ4_vals = [λ[4] for λ in lambdas]
end

# ╔═╡ 43228fce-342d-4eb8-9370-ecaf18847c5f
plot(
    t,
    [λ1_vals λ2_vals λ3_vals λ4_vals],
    xlabel = "Time (s)",
    ylabel = "Constraint Force / Multiplier",
    title = "Constraint Forces",
    label = ["Track y₁" "Block θ₁" "Pin x" "Pin y"]
)

# ╔═╡ b4e798f2-516a-4c2b-9bd1-ef4016f821cd
begin
    anim = @animate for i in 1:5:length(sol.t)
        ...
        tip_x = pin_x + L*cos(θ2)
        tip_y = pin_y + L*sin(θ2)
        ...
        plot(
            ...
            aspect_ratio = :equal,
            legend = false,
            title = "Spring-Block-Pendulum Motion",
            xlabel = "x (m)",
            ylabel = "y (m)"
        )

        # ground line
        plot!([-0.4, 0.4], [-0.03, -0.03], linewidth = 2)

        # block
        plot!(
            Shape(
                [x1 - 0.04, x1 + 0.04, x1 + 0.04, x1 - 0.04],
                [y1 - 0.02, y1 - 0.02, y1 + 0.04, y1 + 0.04]
            ),
            alpha = 0.5
        )

        # spring line
        plot!([-0.35, x1 - 0.04], [0.0, 0.0], linewidth = 2)

        # bar
        plot!([pin_x, tip_x], [pin_y, tip_y], linewidth = 5)

        # pin
        scatter!([pin_x], [pin_y], markersize = 5)
    end

    gif(anim, "spring_block_pendulum.gif", fps = 30)
end
